...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    api_instance = openapi_client.TrashcanApi(api_client)
...
> file get_archived_node_rendition_content(node_id, rendition_id, attachment=attachment, if_modified_since=if_modified_since, range=range, placeholder=placeholder)
...
> file get_deleted_node_content(node_id, attachment=attachment, if_modified_since=if_modified_since, range=range)
...
deleted_node_body_restore = openapi_client.DeletedNodeBodyRestore() # DeletedNodeBodyRestore | The targetParentId if the node is restored to a new location. (optional)
